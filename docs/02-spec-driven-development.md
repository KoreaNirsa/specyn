...
특히 `agent.md`는 단순 설명서가 아니라 **실제 multi-agent 실행 흐름**의 기준점이다.
...
| spec | 역할 |
|---|---|
| product.md | 문제 정의, 사용자, 시나리오, NFR, 제외 범위 |
| api.md | endpoint, schema, error contract, 운영 제약, 구조 규칙(`global/common/domain`) |
| test.md | 성공/실패/검증/coverage 기준 |
| review.md | release 품질 기준, blocker / major / minor 규칙 |
| agent.md | 어떤 agent를 어떤 순서로 실행할지, handoff/stop/retry 규칙, feedback loop 상한 |
...
1. `product.md`는 downstream spec의 어휘와 제약을 고정한다.
2. `api.md`는 Backend/Frontend/Test/Docs가 함께 쓰는 계약이다.
3. `test.md`와 `review.md`는 생성 이후 품질 게이트 역할을 한다.
4. `agent.md`는 실제 multi-agent 흐름을 정의하므로 실행 코드와 정합해야 한다.
...
7. `agent.md`의 `feedback_loops`와 `max_feedback_rounds`를 사용하면 bounded multi-agent collaboration을 설계할 수 있다.
