...
기획부터 스펙 관리, 코드 생성, 테스트, 리뷰, 문서화, 그리고 CI 검증까지 이어지는 **SDD(Spec Driven Development) 기반의 프로덕션 지향 골격**을 제공합니다.
...
- `specyn run` 기본 로컬 런타임으로 frontend/backend/ai-server/docs 산출물을 직접 생성할 수 있습니다.
- Backend + AI Server + Codex CLI 연동 모드로 실제 patch/file 생성 경로를 확장할 수 있습니다.
...
### `compile-prompts` 다음 단계: 전체 SDD 실행

Specyn은 여기서 끝나지 않습니다. `specyn run` 기본 로컬 런타임을 사용하면 스펙을 바탕으로 실제 저장소 산출물을 바로 생성할 수 있습니다.
...
`python scripts/specyn_tasks.py dev` 실행 후에는 아래 주소로 결과를 바로 확인할 수 있습니다.
...
Backend + AI Server + Codex CLI 경유 모드도 지원합니다. 이 경우에는 아래처럼 `--backend-url` 과 `--workspace` 를 함께 사용합니다.
...
- `compile-prompts` 이후 전체 SDD 흐름을 보려면 [적용 플레이북](docs/playbook.md)을 보시면 됩니다.
- Codex 환경을 붙이려면 [Codex 실행 문서](docs/04-codex-execution.md)를 보시면 됩니다.
...
Specyn은 단순한 코드 생성 도구가 아니라 **팀의 개발 프로세스를 스펙 중심으로 정렬해 주는 프레임워크**에 가깝습니다.
