...
def test_ai_server_client_uses_http_client_builder_for_connect_timeout() -> None:
    content = (
        ROOT / "src" / "main" / "java" / "com" / "axbuilder" / "backend" / "client" / "AiServerClient.java"
    ).read_text(encoding="utf-8")

    assert "HttpClient.newBuilder()" in content
    assert ".connectTimeout(this.timeout)" in content
    assert "HttpRequest.newBuilder()" in content
    assert ".timeout(timeout)" in content


def test_security_config_disables_generated_dev_user_when_security_is_off() -> None:
    content = (
        ROOT / "src" / "main" / "java" / "com" / "axbuilder" / "backend" / "config" / "SecurityConfig.java"
    ).read_text(encoding="utf-8")
