...
def test_local_runtime_generates_repo_artifacts(monkeypatch, tmp_path: Path) -> None:
    ...
    for path in [frontend_contract, frontend_page, backend_controller, ai_router, generated_doc, run_manifest]:
        assert path.exists(), path

    frontend_page_text = frontend_page.read_text(encoding="utf-8")
    assert 'key={endpoint.method + "-" + endpoint.path}' in frontend_page_text
    assert "specyn run --spec-dir ..." in frontend_page_text
